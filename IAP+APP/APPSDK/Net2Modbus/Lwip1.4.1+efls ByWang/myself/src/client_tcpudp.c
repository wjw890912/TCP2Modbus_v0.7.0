...
extern uint8_t TCP2RS485CMDLIST[1024];	//用来接收来自服务器的轮训发送数据帧
		 ...
		 data = p->payload;	 
					...
					if (strncmp(data, Data, 2) == 0)
					 {
					   	...
					   	if (strncmp(data, Data1, 2) == 0)
						{
							 ...
							 TcpRecvLenth=p->tot_len-5-2;//upadta the recive data lenth	     BB FF  24 0D 0A =5 BYTES
					...
					if (strncmp(data, "mem", 3) == 0)
					 ...
					 if (strncmp(data, "getid", 5) == 0)
						...
						else if (strncmp(data, "reboot", 6) == 0)
						   {
						   	   //系统重启
